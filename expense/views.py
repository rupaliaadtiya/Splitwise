...
class CreateExpenseApiView(APIView):
    ...
    def post(self, request) -> Response:
        try:
            ...
            if amount > 100000000:
                return Response({"message": "Expense amount exceeds the maximum limit"}, status=status.HTTP_400_BAD_REQUEST)
            if len(all_users) > 1000:
                return Response({"message": "Number of participants exceeds the limit of 1000"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if split_type == 'equal':
                ...
                if not percentages or len(percentages) != len(all_users):
                    return Response({"message": "Invalid percentage distribution"}, status=status.HTTP_400_BAD_REQUEST)

                total_percentage = sum(percentages)
                if total_percentage != 100:
                    return Response({"message": "Percentages must add up to 100"}, status=status.HTTP_400_BAD_REQUEST)
                ...
                if not exact_amounts or len(exact_amounts) != len(all_users):
                    return Response({"message": "Invalid exact amount distribution"}, status=status.HTTP_400_BAD_REQUEST)
